...
<#
.SYNOPSIS
    Claude Code + OpenClaw Backup v27.0 - HARDLINK INCREMENTAL (50x faster)
.DESCRIPTION
    Two-phase backup: robocopy incremental into persistent "latest" dir (only changed
    files copied), then hardlink-clone to timestamped snapshot (instant, zero extra disk).
    v27.0: Persistent latest dir + hardlink snapshots = true 50x speedup on repeat runs.
...
Write-Host "  CLAUDE CODE + OPENCLAW BACKUP v27.0 HARDLINK INCREMENTAL" -ForegroundColor White
Write-Host "  INCREMENTAL TO LATEST | HARDLINK SNAPSHOT | ALL PARALLEL" -ForegroundColor Yellow
Write-Host ("=" * 80) -ForegroundColor Cyan
...
# Phase B: hardlink-clone "latest" -> timestamped snapshot (instant, zero I/O)
...
# $BackupPath will be created at the end via hardlink clone
...
$copyScript = {
    ...
    try {
        ...
        if (Test-Path $src -PathType Container) {
            ...
            $argStr = "`"$src`" `"$dst`" /E /XO /R:0 /W:0 /MT:128 /NFL /NDL /NJH /NJS /NP /FFT"
            if ($xdList.Count -gt 0) {
                $quoted = @(); foreach ($x in $xdList) { $quoted += "`"$x`"" }
                $argStr += " /XD " + ($quoted -join " ")
            }
            $proc = Start-Process -FilePath "robocopy" -ArgumentList $argStr -WindowStyle Hidden -PassThru
...
Add-Task "F:\study\AI_ML\AI_and_Machine_Learning\Artificial_Intelligence\cli\claudecode\wrappers\ClawdBot" "$LatestDir\openclaw\clawdbot-wrappers" "ClawdBot wrappers" 30
...
if($env:SKIP_PROJECT_SEARCH -eq "1"){
    ...
    if($projDirs.Count -gt 0){
        ...
        $p4Script={
            ...
            Start-Process -FilePath "robocopy" -ArgumentList "`"$src`" `"$dst`" /E /XO /R:0 /W:0 /MT:128 /NFL /NDL /NJH /NJS /NP /FFT /XD node_modules .git" -WindowStyle Hidden -Wait
            return $label
        }
        ...
        foreach($dir in $projDirs){
            ...
            $dst="$LatestDir\project-claude\$sn"; $label="$($dir.Parent.Name)\.claude"
            $ps=[System.Management.Automation.PowerShell]::Create()
...
"@
    ...
    # Create directory structure first (instant)
    foreach ($d in [System.IO.Directory]::EnumerateDirectories($LatestDir, '*', [System.IO.SearchOption]::AllDirectories)) {
        $rel = $d.Substring($LatestDir.Length)
        $target = $BackupPath + $rel
        if (-not [System.IO.Directory]::Exists($target)) {
            [System.IO.Directory]::CreateDirectory($target) | Out-Null
        }
    }
    # Hardlink every file via kernel32 (no process spawn per file)
    $hlCount = 0; $hlFail = 0
    $latLen = $LatestDir.Length
    foreach ($f in [System.IO.Directory]::EnumerateFiles($LatestDir, '*', [System.IO.SearchOption]::AllDirectories)) {
        $rel = $f.Substring($latLen)
        $target = $BackupPath + $rel
        if ([HardLinkHelper]::CreateHardLink($target, $f, [IntPtr]::Zero)) {
            $hlCount++
        } else {
            # Fallback: copy if hardlink fails (cross-volume, non-NTFS, locked)
            try { [System.IO.File]::Copy($f, $target, $true); $hlCount++ } catch { $hlFail++ }
        }
    }
    $hlSw.Stop()
    Write-Host "[P8] $hlCount hardlinks ($hlFail fallback) in $([math]::Round($hlSw.Elapsed.TotalSeconds,1))s" -ForegroundColor Green
} catch {
...
# Metadata (inside both latest and snapshot)
try {
    $metaJson = @{
        Version   = "27.0 HARDLINK INCREMENTAL"
