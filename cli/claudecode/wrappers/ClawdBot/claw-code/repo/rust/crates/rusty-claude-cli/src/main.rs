...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Repl {
            ...
            permission_mode,
            base_commit,
        } => run_repl(model, model_explicit, allowed_tools, permission_mode, base_commit)?,
...
fn resolve_repl_model(cli_model: String, explicit: bool) -> String {
    if explicit {
        return cli_model;
    }
...
fn run_repl(
    ...
    run_stale_base_preflight(base_commit.as_deref());
    let resolved_model = resolve_repl_model(model, model_explicit);
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn render_status_bar(&self) {
        ...
        // --- Line 1: 5h/7d usage + context % to compact ---
        let mut win5 = String::new();
        let mut win7 = String::new();
        let usage_path = claude_dir.join("usage-cache.json");
        if let Ok(data) = std::fs::read_to_string(&usage_path) {
            if let Ok(parsed) = serde_json::from_str::<Value>(&data) {
                if let Some(windows) = parsed.get("windows").and_then(|w| w.as_array()) {
                    ...
                    for w in windows {
                        ...
                        let util = w.get("utilization").and_then(|u| u.as_f64()).unwrap_or(0.0);
                        let reset = w.get("reset_ts").and_then(|r| r.as_f64()).unwrap_or(0.0) as u64;
                        let pct = (util * 100.0).round() as u32;
                        let diff = reset.saturating_sub(now);
                        let time_str = if diff == 0 {
                            "now".to_string()
                        } else {
                            let days = diff / 86400;
                            let hrs = (diff % 86400) / 3600;
                            let mins = (diff % 3600) / 60;
                            if days > 0 { format!("{days}d {hrs}h") }
                            else if hrs > 0 { format!("{hrs}h {mins}m") }
                            else { format!("{mins}m") }
                        };
                        match key {
                            "5h" => win5 = format!("5h:{pct}%({time_str})"),
                            "7d" => win7 = format!("7d:{pct}%({time_str})"),
                            _ => {}
                        }
                    }
                }
        ...
        // Context % to compact from resource-config.json
        let rc_path = claude_dir.join("resource-config.json");
        let mut compact_at: u64 = 970_000;
        if let Ok(data) = std::fs::read_to_string(&rc_path) {
            if let Ok(parsed) = serde_json::from_str::<Value>(&data) {
                if let Some(ca) = parsed.get("compactAt").and_then(|c| c.as_u64()) {
                    compact_at = ca;
                }
            }
        }
        let est_k = estimated as f64 / 1000.0;
        let compact_k = compact_at as f64 / 1000.0;
        let ctx_pct = if compact_at > 0 {
            ((estimated as f64 / compact_at as f64) * 100.0).round() as u32
        } else { 0 };
        let ctx_remaining = 100_u32.saturating_sub(ctx_pct);

        // Color 5h green, 7d yellow
        let line1 = if win5.is_empty() && win7.is_empty() {
            format!(
                " \x1b[36m{ctx_remaining}% to compact {est_k:.0}k/{compact_k:.0}k\x1b[0m"
            )
        } else {
            // Color the % in each window
            format!(
                " \x1b[32m{win5}\x1b[0m | \x1b[33m{win7}\x1b[0m | {ctx_remaining}% to compact {est_k:.0}k/{compact_k:.0}k"
            )
        };

        // --- Line 2: CPU + GPU + RAM from hw cache ---
        let mut cpu_pct = String::from("--");
        let mut gpu_pct = String::from("--");
        let mut ram_used = String::from("--");
        let mut ram_total = String::from("--");
        let hw_cache = PathBuf::from("/tmp/claude_statusline_hw");
        if let Ok(data) = std::fs::read_to_string(&hw_cache) {
            let lines: Vec<&str> = data.lines().collect();
            if lines.len() >= 4 {
                cpu_pct = lines[0].trim().to_string();
                gpu_pct = lines[1].trim().to_string();
                ram_used = lines[2].trim().to_string();
                ram_total = lines[3].trim().to_string();
            }
        }

        println!("{line1}");
        println!(
            " cpu:{cpu_pct}% | gpu:{gpu_pct}% | ram:{ram_used}g/{ram_total}g"
        );
    }
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        let has_bearer = auth.bearer_token().is_some();
        let mut client = AnthropicClient::from_auth(auth)
...
mod tests {
    ...
    fn resolve_repl_model_returns_user_supplied_model_unchanged_when_explicit() {
        ...
        let resolved = resolve_repl_model(user_model, true);
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        let resolved =
            with_current_dir(&root, || resolve_repl_model(DEFAULT_MODEL.to_string(), false));
