...
impl AnthropicClient {
    ...
    pub fn from_env() -> Result<Self, ApiError> {
        ...
        let mut client = Self::from_auth(auth).with_base_url(read_base_url());
        if has_bearer {
    ...
    async fn send_with_retry(
        ...
        loop {
            ...
            match self.send_raw_request(request).await {
                Ok(response) => {
                    let is_429 = is_rate_limited(response.status());
                    match expect_success(response).await {
                        ...
                        Err(error) if is_429 && rate_limit_retries < MAX_RATE_LIMIT_RETRIES => {
                            rate_limit_retries += 1;
                            let wait_secs = 3 + rate_limit_retries * 2;
                            eprintln!(
                                "\x1b[33m⏳ Rate limited (429). Retrying in {wait_secs}s... ({rate_limit_retries}/{MAX_RATE_LIMIT_RETRIES})\x1b[0m"
                            );
                            self.record_request_failure(attempts, &error);
                            last_error = Some(error);
                            tokio::time::sleep(Duration::from_secs(wait_secs.into())).await;
                            continue;
                        }
...
const fn is_rate_limited(status: reqwest::StatusCode) -> bool {
    status.as_u16() == 429
}
